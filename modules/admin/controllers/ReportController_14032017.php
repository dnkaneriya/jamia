...
//        print_r($det);exit;
    ...
    public function actionHostelRoomReport() {
        ...
        $query1 = new Query();
        $query1->select(["CONCAT(`sm`.`surname_en`,' ',`sm`.`firstname_en`,' ',`sm`.`lastname_en`) AS fullname"])->from('hostel_master h')->innerjoin('student_master sm', 'h.student_id = sm.id')->where(['h.room_id' => $room_no, 'h.is_monitor' => '1']);
        $hostel_data1 = $query1->createCommand()->queryAll();
        $monitor = $hostel_data1[0]['fullname'];
    ...
    public function actionGetStandardList($class_id, $subclass_id)
    {
        $standards = SchoolExam::findAll(['is_deleted' => 'N', 'class_id' => $class_id, 'subclass_id' => $subclass_id]);
