...
class StudentController extends Controller
{
    ...
    /*public function actionExport()
    {
        ...
        $output = "";
        if($str){
            $students = Student::find()->where(['is_deleted'=>'N'])->andWhere('id IN('.$str.')')->all();
        }else{
            $students = Student::find()->where(['is_deleted'=>'N'])->all();
        }
        //echo "<pre>";print_r($candidate);die;
        if($_REQUEST['name'] != '' || $_REQUEST['state'] != '' || $_REQUEST['district'] != '' || $_REQUEST['i_date'] != ''){
			//$students = Student::find()->where(['is_deleted'=>'N'])->andwhere(['or',['like','surname_en',$_REQUEST['name']],['like','firstname_en',$_REQUEST['name']],['like','lastname_en',$_REQUEST['name']]])->all();
			$students = Student::find()->where(['is_deleted'=>'N']);
			if($_REQUEST['name']!= ''){
			$students->andwhere(['or',['like','surname_en',$_REQUEST['name']],['like','firstname_en',$_REQUEST['name']],['like','lastname_en',$_REQUEST['name']]]);
			}
			if($_REQUEST['state']!= ''){
				$students->andwhere(['state'=>$_REQUEST['state']]);
			}
			if($_REQUEST['district']!= ''){
				$students->andwhere(['district'=>$_REQUEST['district']]);
			}
			if($_REQUEST['i_date']!= ''){
				 $sd=strtotime($_REQUEST['i_date']);
				$students->andwhere(['i_date'=>$sd]);
			}
			$students->all();
		}else{
				$students = Student::find()->where(['is_deleted'=>'N'])->all();
