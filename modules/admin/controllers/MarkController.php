...
class MarkController extends Controller {
    ...
    public function actionIndex() {
        if(Yii::$app->user->can('islamic_exam')) {
            ...
            $examList = ArrayHelper::map(ExamMaster::findAll(['is_active' => 'Y', 'is_deleted' => 'N']), 'id', 'name');
        ...
        if(Yii::$app->user->can('islamic_exam')) {
            ...
            if ($postdata = Yii::$app->request->post()) {
                ...
                for ($i = 0; $i < count($students); $i++) {
                    ...
                    $result = ResultMaster::findOne(['class_id' => $postdata['class_id'], 'subclass_id' => $postdata['subclass_id'], 'division_id' => $postdata['division_id'], 'student_id' => $modelMark->student_id, 'year' => $postdata['year'], 'exam_id' => $postdata['exam_id']]);
                    ...
                    $result->exam_id = $postdata['exam_id'];
...
//                        if($res == 'P') {
//                            $exam = ExamMaster::findOne(['id' => $postdata['exam_id']]);
//                            if($exam->class_upgrade == 1) {
//                                $studentObj = Student::findOne(['id' => $result->student_id]);
//                                if(!empty($studentObj) && count($studentObj) > 0) {
//                                    $classupgradeObj = ClassUpgradeMaster::findOne(['class_id' => $studentObj->class_id, 'subclass_id' => $studentObj['sub_class_id']]);
//                                    if(!empty($classupgradeObj) && count($classupgradeObj) > 0) {
//                                        $studentObj->class_id = $classupgradeObj->upgrade_id;
//                                        $studentObj->sub_class_id = $classupgradeObj->upgrade_subclass_id;
//                                        $studentObj->save();
//                                    }
//                                }
//                             }
//                        }
            ...
            $studentList = ArrayHelper::map(Student::find()->where(['is_deleted' => 'N', 'is_active' => 'Y', 'closed_student' => 0, 'class_id' => $class_id, 'sub_class_id' => $subclass_id, 'divison_id' => $division_id])->andWhere(['<>', 'grno', ''])->all(), 'id', 'grno');
