...
function submitForm()
{
    ...
    if(strvalue!=""){
        document.getElementById('delete').href = '<?php echo Yii::$app->request->baseUrl;?>/admin/category/change?str='+strvalue+'&&field=is_deleted&&val=Y';
    }else{
		...
		<div class="box">
			...
			<div class="row p-a">
			  ...
			  <div class="col-sm-3 text-right">
				...
				<a href="<?=Yii::$app->request->baseurl?>/admin/category/create" id="job-export" class="btn btn-fw primary" title="Create a class"><i class="fa fa-plus"></i> Add New</a> 
			  </div>
			</div>
			...
			<?php $layout = '<div class="btn-group pull-right">{toolbar}</div><div class="clearfix"></div>{items}<div class="text-right">{pager}</div>';
					...
					'layout' => $layout,
					'captionOptions'=>['title'=>'Staff Category List'],
					'columns' => [
						[
						...
						'width'=>'10%',
							...
							'urlCreator' => function ($action, $model, $key, $index) {
                                if ($action === 'view') {
                                        $url =Yii::$app->request->baseUrl.'/admin/category/view?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'update') {
                                        $url = Yii::$app->request->baseUrl.'/admin/category/update?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'delete') {
