...
$this->title = Yii::$app->params['apptitle'].' : Home Page Banner List';
...
function submitForm()
{
    ...
    if(strvalue!=""){
        ...
        document.getElementById('delete').href = '<?php echo Yii::$app->request->baseUrl;?>/admin/homebanners/change?str='+strvalue+'&&field=is_deleted&&val=Y';
    }else{
		...
		<div class="box">
			...
			<div class="row p-a">
			  ...
			  <div class="col-sm-5 text-right">
                <a href="<?=Yii::$app->request->baseurl?>/admin/homebanners/create" id="job-export" class="btn btn-fw primary" title="Create a class"><i class="fa fa-plus"></i> Add New</a>
			  </div>
			</div>
			...
			<?php $layout = '<div class="btn-group pull-right">{toolbar}</div><div class="clearfix"></div>{items}<div class="text-right">{pager}</div>';
					...
					'layout' => $layout,
					'captionOptions'=>['title'=>'Banner List'],
					'columns' => [
						[
                        ...
                        [
                            ...
                            'value'=>function($model){
                                if(isset($model->banner) && $model->banner != null){
                                    return '<img src="'.Url::to('@web/'.$model->banner).'" width="100" height="100"/>';
                                }else{
                                    return '';
                                }
                            }
						...
						[
							...
							'urlCreator' => function ($action, $model, $key, $index) {
                                if ($action === 'view') {
                                        $url =Yii::$app->request->baseUrl.'/admin/homebanners/view?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'update') {
                                        $url = Yii::$app->request->baseUrl.'/admin/homebanners/update?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'delete') {
