...
$this->title = Yii::$app->params['apptitle'] . ' : School Exam';
...
//function for change pagination 
    ...
    function submitForm()
    {
        ...
        if (strvalue != "") {
            document.getElementById('delete').href = '<?php echo Yii::$app->request->baseUrl; ?>/admin/room/change?str=' + strvalue + '&&field=is_deleted&&val=Y';
        } else {
        ...
        <div class="box">
            ...
            <div class="row p-a">
                ...
                <div class="col-sm-3 text-right">
                    ...
                    <a href="<?= Yii::$app->request->baseurl ?>/admin/school-exam/create" id="job-export" class="btn btn-fw primary" title="Add New"><i class="fa fa-plus"></i> Add New</a> 
                </div>
            </div>
            ...
            echo GridView::widget([
                ...
                'columns' => [
                        ...
                        'value'=>function($model){
                                $className = Classes::find()->where(['id'=>$model->class_id])->one();
                                if($className){
                        ...
                        'label' => 'Subclass',
                        'value'=>function($model){
                                $subclassName = Subclass::find()->where(['id'=>$model->subclass_id])->one();
                                if($subclassName){
                                    return $subclassName->sub_class;
                                }else{
                                    return '';
                                }
                            }    
                    ...
                    [
                        ...
                        'urlCreator' => function ($action, $model, $key, $index) {
                            if ($action === 'view') {
                                $url = Yii::$app->request->baseUrl . '/admin/school-exam/view?id=' . $model->id;
                                return $url;
                            }
                            if ($action === 'update') {
                                $url = Yii::$app->request->baseUrl . '/admin/school-exam/update?id=' . $model->id;
                                return $url;
                            }
                            if ($action === 'delete') {
