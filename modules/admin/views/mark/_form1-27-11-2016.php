        	...
        	<div class="col-sm-8">
        		...
        		<?= $form->field($model, 'division_id')->dropDownList(
        			...
        			[
                        ...
                        'onchange' => '
                                $( "select#mark-student_id" ).html("showLoading");
                                $.post( "get-division-list?class_id='.'"+$("#mark-class_id").val()+"&subclass_id="+$("#mark-subclass_id").val()+"&division_id="+$(this).val(), 
                                function(data) {
                                        $( "select#mark-student_id" ).html(data);
                                });'
    				]
            ...
            <div class="col-sm-8">
                ...
                <?= $form->field($model, 'student_id')->dropDownList(
                    ...
                    [
                        ...
                        'onchange' => '
                                    var exam_id = $("#mark-exam_id").val();
                                    var exam_year = $("#mark-year").val();
                                    //alert(exam_id);
                                    //alert(exam_year);
                                    if(exam_id && exam_year) {
                                        $( ".dispdata" ).html("Loading...");
                                        $.post( "getsubdata?id='.'"+$(this).val()+"&year="+exam_year+"&exam_id="+exam_id, 
                                        function(data) {
                                                $( ".dispdata" ).html(data);
                                        });    
                                    } else {
                                        $( ".dispdata" ).html("<div class=alert>Please select Exam and Year First!</div>");
                                    }'
