...
<?php echo Yii::t('app', 'You have some form errors. Please check below'); ?>
            ...
            <div class="form-group field-mark-grno required">
                <div class="row"><label class="control-label col-sm-2 form-control-label" for="mark-grno">GR No</label>
                    <div class="col-lg-4">
                        <input list="grno" id="mark-grno" class="form-control has-value" data-id="<?= $model->id ?>" name="Mark[grno]" value="<?php echo $model->grno; ?>" maxlength="255" placeholder="GR No">
                        <datalist id="grno">
<?php
if ($student_data != array()) {
    foreach ($student_data as $data) {
        ?>
                                    <option value="<?= $data->grno ?>">
                                    <?php }
                                }
                                ?>
                        </datalist>
