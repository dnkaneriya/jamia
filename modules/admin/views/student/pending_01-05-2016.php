...
$this->title = Yii::$app->params['apptitle'].' : Pending Student List';
...
function assign_gr()
{
    ...
    if(strvalue!="" && series!=""){
        document.getElementById('student_select').href = '<?php echo Yii::$app->request->baseUrl;?>/admin/student/selected?str='+strvalue+'&&series='+series;
    }
}
...
function exportcsv()
{
    ...
    var name=$('input[name="StudentSearch[full_name]"]').val();
    var state=$('input[name="StudentSearch[state]"]').val();
    var district=$('input[name="StudentSearch[district]"]').val();
    var i_date=$('input[name="StudentSearch[i_date]"]').val();
    //if(strvalue!="")
    //{
        //document.getElementById('deactive').href = '<?php echo Yii::$app->request->baseUrl;?>/user/change?str='+strvalue+'&&field=is_active&&val=N';
        //document.getElementById('active').href = '<?php echo Yii::$app->request->baseUrl;?>/user/change?str='+strvalue+'&&field=is_active&&val=Y';
        document.getElementById('student-export').href = '<?php echo Yii::$app->request->baseUrl;?>/admin/student/export?str='+strvalue+'&&redirect=pending&&name='+name+'&&state='+state+'&&district='+district+'&&i_date='+i_date+'';
    /*}
    else
    {
	//document.getElementById('deactive').href = 'javascript:void(0);';
	document.getElementById('candidate-export').href = 'javascript:void(0);';
	//document.getElementById('active').href = 'javascript:void(0);';
    }*/
	...
	<div class="padding">
		...
		<div class="box">
			...
			<div class="row p-a">
			  ...
			  <div class="col-sm-4">
               <!-- <input type="number" min="0" class="gr_assign" name="series" id="series_value" placeholder="Series Number">
                <a href="javascript:void(0);" class="btn btn-fw primary" onclick="assign_gr();" id="student_select">Select</a> -->
			  ...
			  <div class="col-sm-5 text-right">
                ...
                <a href="javascript:void(0);" id="student-export" class="btn btn-fw primary" title="Dowload csv file" onclick="exportcsv();"><i class="fa fa-download"></i> Export</a> 
                        ...
                        <?php
							...
							$classes = Classes::find()->where(['is_deleted'=>'N'])->all();
                            $classList = ArrayHelper::map($classes, 'id', 'name');
			...
			<?php $layout = '<div class="btn-group pull-right">{toolbar}</div><div class="clearfix"></div>{items}<div class="text-right">{pager}</div>';
					...
					'layout' => $layout,
					'captionOptions'=>['title'=>'Pending Student List'],
					'columns' => [
						[
