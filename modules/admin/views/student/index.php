...
function assign_tajwidclass()
{
    ...
    var tajwidclass = document.getElementById('tajwidclass').value;
    if(strvalue == "")
	{
		alert('Please select atleast one record');
		return false;
    ...
    if(strvalue!="" && tajwidclass!=""){
       $.ajax({
        type:"GET",
        url:"allocatetajwidclass",
        data:{str:strvalue,tajwidclass:tajwidclass},    // multiple data sent using ajax
        success: function (result) {
            $.pjax.reload({container: '#w0-pjax', timeout: 2000});
        }
   		 });
...
function assign_standard()
{
    ...
    if(strvalue!="" && standard!=""){
       $.ajax({
        type:"GET",
        url:"allocatestandard",
        data:{str:strvalue,standard:standard},    // multiple data sent using ajax
        success: function (result) {
            $.pjax.reload({container: '#w0-pjax', timeout: 2000});
        }
   		 });
...
function student_certi(certi)
{
    ...
    if(strvalue!=""){
       $.ajax({
        type:"GET",
        url:"studentcerti",
        data:{str:strvalue, certi:certi},    // multiple data sent using ajax
        success: function (result) {
            $.pjax.reload({container: '#w0-pjax', timeout: 2000});
        }
   		 });
		...
		<div class="box">
			...
			<div class="row p-a">
              ...
              <?php
                 ...
                 <a href="javascript:void(0);" id="student-export" class="btn btn-fw primary" title="Dowload csv file" onclick="exportcsv();"><i class="fa fa-download"></i> Export</a>
                    ...
                    <div class="row p-a">
                        <div class="col-sm-10 col-sm-offset-2">
                            <a href="javascript:void(0);" class="btn btn-fw primary" onclick="student_certi('hafiz');" id="close_student">Hafiz</a>
                            <a href="javascript:void(0);" class="btn btn-fw primary" onclick="student_certi('aalim');" id="close_student">Aalim</a>
                            <a href="javascript:void(0);" class="btn btn-fw primary" onclick="student_certi('close');" id="close_student">Close</a>
                            <?php
				
				if($tajwidClassList != array()){?>
                <select name="subclass" id="tajwidclass" style="padding: 8px;">
                	<option value="">Select Tajwid Class</option>
					<?php
					foreach($tajwidClassList as $tajwidClass):
					?>
                    <option value="<?=$tajwidClass->id?>"><?=$tajwidClass->class_name?></option>
					<?php
					endforeach;
					?>
					</select>
				...
				?>
                <a href="javascript:void(0);" class="btn btn-fw primary" onclick="assign_tajwidclass();" id="assign_tajwidclass">Submit</a>
              
              <?php
				
				if($standardList != array()){?>
                <select name="division" id="standard" style="padding: 8px;">
                ...
                <a href="javascript:void(0);" class="btn btn-fw primary" onclick="assign_standard();" id="assign_standard">Submit</a>
...
//                        ],
						...
						[
                            ...
                            'headerOptions' => ['style' => 'text-align:center'],
                            'template' => '{update}&nbsp;{delete}&nbsp;{view}',//{view}&nbsp;
                            'buttons' => [
                                'view' => function ($url, $model){
                                    return Html::a('<button class="btn btn-info ajaxupdate" style="padding:5px 10px;"><i class="fa fa-file-pdf-o"></i></button>', $url, [
                                        'title' => Yii::t('app', 'PDF'),'data-pjax' => true,'target'=>'_blank'
                                    ]);
                                },
							...
							'urlCreator' => function ($action, $model, $key, $index) {
                                if ($action === 'view') {
                                        $url =Yii::$app->request->baseUrl.'/admin/student/studentdetailpdf?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'update') {
