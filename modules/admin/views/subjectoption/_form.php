    ...
    <div class="col-lg-12">
		<!--<section class="panel">-->
            <div class="box-header">
				<?php
					if($model->isNewRecord) {
						echo '<h3>'.Yii::t('app', 'Add Tarbiyat Subject Option').'</h3>';
					}else{
						echo '<h3>'.Yii::t('app', 'Edit Tarbiyat Subject Option').'</h3>';
					}
				?>
            </div>
            ...
            <div class="box-body">
                ...
                ?>
                <?= $form->field($model, 'tarbiyat_subject_id')->dropDownList($listData, ['prompt' => '-Select Tarbiyat Subject-']); ?>
				<div class="form-group field-subjectoption-options">
					<div class="row">
						<label class="col-sm-2 form-control-label" for="subjectoption-options">Subject Options</label>
						<div class="col-lg-4">
							<select id="subjectoption-options" class="form-control" name="Subjectoption[options]">
								<?php
									if(!$model->isNewRecord && $model->options == 'A'){ $a = 'selected="selected"'; $b = ''; $c = ''; $d = ''; }
									elseif(!$model->isNewRecord && $model->options == 'B'){ $a = ''; $b = 'selected="selected"'; $c = ''; $d = ''; }
									elseif(!$model->isNewRecord && $model->options == 'C'){ $a = ''; $b = ''; $c = 'selected="selected"'; $d = ''; }
									elseif(!$model->isNewRecord && $model->options == 'D'){ $a = ''; $b = ''; $c = ''; $d = 'selected="selected"'; }
									else{$a = ''; $b = ''; $c = ''; $d = ''; }
								?>
								<option value="">--Select Subject Options--</option>
								<option value="A" <?=$a?>>Regular without any person tell</option>
								<option value="B" <?=$b?>>Regular but in month some time lazy</option>
								<option value="C" <?=$c?>>Regular but always lazy</option>
								<option value="D" <?=$d?>>Not Regular</option>
							</select>
...
<script type="text/javascript">
	var form1 = $('#tarbiyatsubjectoption-form');
	...
	form1.validate({
		...
		rules: {
            "Subjectoption[tarbiyat_subject_id]": {
				required: true
			},
			"Subjectoption[options]": {
				required: true
			},
        },
