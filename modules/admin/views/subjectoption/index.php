...
$this->title = Yii::$app->params['apptitle'].' : Tarbiyat Subject Option';
...
function submitForm()
{
    ...
    if(strvalue!=""){
        ...
        document.getElementById('delete').href = '<?php echo Yii::$app->request->baseUrl;?>/admin/subjectoption/change?str='+strvalue+'&&field=is_deleted&&val=Y';
    }else{
	...
	<div class="padding">
		...
		<div class="box">
			<div class="box-header">
			  <h3>Tarbiyat Subject Options</h3>
			</div>
			<div class="row p-a">
			  <div class="col-sm-5">
				<?php
			  ...
			  <div class="col-sm-3 text-right">
				<a href="<?=Yii::$app->request->baseurl?>/admin/subjectoption/create" id="job-export" class="btn btn-fw primary" title="Create a class"><i class="fa fa-plus"></i> Add New</a>
			  </div>
			</div>
            <?php
                $tSubjectList = ArrayHelper::map(Tarbiyatsubject::find()->where(['is_deleted'=>'N'])->all(), 'id', 'subject');
                $subjectOptionList = ArrayHelper::map(Subjectoption::find()->where(['is_deleted'=>'N'])->all(), 'options', 'options');
                $array = array(
                                'A'=>'Regular without any person tell', 'B'=>'Regular but in month some time lazy',
                                'C'=>'Regular but always lazy', 'D'=>'Not Regular'
                            );
                foreach($subjectOptionList as $key=>$value){
                    $option[$key] = $array[$value];
                }
            ?>
			...
			<?php $layout = '<div class="btn-group pull-right">{toolbar}</div><div class="clearfix"></div>{items}<div class="text-right">{pager}</div>';
					...
					'layout' => $layout,
					'captionOptions'=>['title'=>'Tarbiyat Subject Option'],
					'columns' => [
						[
                        ...
                        [
                            ...
                            'value'=>function($model){
                                ...
                                if(isset($model->options) && $model->options != null){
						...
						[
							...
							'urlCreator' => function ($action, $model, $key, $index) {
                                if ($action === 'view') {
                                        $url =Yii::$app->request->baseUrl.'/admin/subjectoption/view?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'update') {
                                        $url = Yii::$app->request->baseUrl.'/admin/subjectoption/update?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'delete') {
