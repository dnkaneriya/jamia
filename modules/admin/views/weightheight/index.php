...
$this->title = Yii::$app->params['apptitle'].' : Weight & Height List';
$this->params['breadcrumbs'][] = $this->title;
$year=array();
for($i=1430;$i<=1600;$i++)
	$year[$i]=$i;
	
$studentList = ArrayHelper::map(Student::find()->where(['is_deleted'=>'N','is_selected'=>'C'])->all(), 'id', 'firstname_en');
?>
<script>
...
function submitForm()
{
    ...
    if(strvalue!=""){
        ...
        document.getElementById('delete').href = '<?php echo Yii::$app->request->baseUrl;?>/admin/weightheight/change?str='+strvalue+'&&field=is_deleted&&val=Y';
    }else{
		...
		<div class="box">
			...
			<div class="row p-a">
			  ...
			  <div class="col-sm-3 text-right">
				<a href="<?=Yii::$app->request->baseurl?>/admin/weightheight/create" id="job-export" class="btn btn-fw primary" title="Create a class"><i class="fa fa-plus"></i> Add New</a>
			  </div>
			</div>
			...
			<?php $layout = '<div class="btn-group pull-right">{toolbar}</div><div class="clearfix"></div>{items}<div class="text-right">{pager}</div>';
					...
					'layout' => $layout,
					'captionOptions'=>['title'=>'Weight & Height List'],
					'columns' => [
						[
                        ...
                        [
                            ...
                            'filter'=>$studentList,
                            'label'=>'Student Name',
                            'value'=>function($model) use($studentList){
                               //return $studentList[$model->student_id];
                               return 0;
						...
						[
							...
							'urlCreator' => function ($action, $model, $key, $index) {
                                if ($action === 'view') {
                                        $url =Yii::$app->request->baseUrl.'/admin/weightheight/view?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'update') {
                                        $url = Yii::$app->request->baseUrl.'/admin/weightheight/update?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'delete') {
