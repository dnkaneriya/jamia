...
$student=Student::find()->where(['is_deleted'=>'N','is_selected'=>'C'])->andwhere('grno != ""')->all();
$studentlist=ArrayHelper::map($student,'grno','grno');
$room=Room::find()->where(['is_deleted'=>'N'])->all();
$roomlist=ArrayHelper::map($room,'id','room_no');
/* @var $this yii\web\View */
/* @var $searchModel app\models\UsersSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$this->title = Yii::$app->params['apptitle'].' : Bed List';
...
function submitForm()
{
    ...
    if(strvalue!=""){
        ...
        document.getElementById('delete').href = '<?php echo Yii::$app->request->baseUrl;?>/admin/hostel/change?str='+strvalue+'&&field=is_deleted&&val=Y';
    }else{
	...
	<div class="padding">
		...
		<div class="box">
			<div class="box-header">
			  <h3>Hostel Informatioin List</h3>
			</div>
			<div class="row p-a">
			  <div class="col-sm-5">
				<?php
			  ...
			  <div class="col-sm-3 text-right">
				...
				<a href="<?=Yii::$app->request->baseurl?>/admin/hostel/create" id="job-export" class="btn btn-fw primary" title="Create a class"><i class="fa fa-plus"></i> Add New</a> 
			  </div>
			</div>
			...
			<?php $layout = '<div class="btn-group pull-right">{toolbar}</div><div class="clearfix"></div>{items}<div class="text-right">{pager}</div>';
					...
					'columns' => [
                        ...
                        [
                            ...
                            'filter'=>$roomlist,
							'value'=>function($model){
								$room=Room::find()->where(['is_deleted'=>'N','id'=>$model->room_id])->one();
								if($room != array())
									return $room->room_no;
							}
                        ],
						[
                            ...
                            'filter'=>true,
							'value'=>function($model){
								$bed=Bed::find()->where(['is_deleted'=>'N','id'=>$model->bed_id])->one();
								if($bed != array())
									return $bed->bed_no;
							}
                        ],						
						[
							...
							'urlCreator' => function ($action, $model, $key, $index) {
                                if ($action === 'view') {
                                        $url =Yii::$app->request->baseUrl.'/admin/hostel/view?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'update') {
                                        $url = Yii::$app->request->baseUrl.'/admin/hostel/update?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'delete') {
