...
$staff=Staff::find()->where(['is_deleted'=>'N'])->all();
$stafflist=ArrayHelper::map($staff,'id','name');
$type=Leavetype::find()->where(['is_deleted'=>'N'])->all();
...
function submitForm()
{
    ...
    if(strvalue!=""){
        ...
        document.getElementById('delete').href = '<?php echo Yii::$app->request->baseUrl;?>/admin/leave/change?str='+strvalue+'&&field=is_deleted&&val=Y';
    }else{
		...
		<div class="box">
			...
			<div class="row p-a">
			  ...
			  <div class="col-sm-3 text-right">
				...
				<a href="<?=Yii::$app->request->baseurl?>/admin/leave/create" id="job-export" class="btn btn-fw primary" title="Create a class"><i class="fa fa-plus"></i> Add New</a> 
			  </div>
			</div>
			...
			<?php $layout = '<div class="btn-group pull-right">{toolbar}</div><div class="clearfix"></div>{items}<div class="text-right">{pager}</div>';
					...
					'layout' => $layout,
					'captionOptions'=>['title'=>'Leave Information List'],
					'columns' => [
						[
						...
						[
							...
							'value'=>function($model){
								$category=Category::find()->where(['is_deleted'=>'N','id'=>$model->staff_cat_id])->one();
								if($category != array())
                        ...
                        [
                            ...
                            'filter'=>$stafflist,
							'value'=>function($model){
								$staff=Staff::find()->where(['is_deleted'=>'N','id'=>$model->staff_id])->one();
								if($staff != array())
									return $staff->name;
							}
                        ],
						[
                            ...
                            'filter'=>$typelist,
							'value'=>function($model){
								$type=Leavetype::find()->where(['is_deleted'=>'N','id'=>$model->type_id])->one();
								if($type != array())
									return $type->name;
							}
                        ],	
						...
						[
                            ...
                            'value'=>function($model){
                                return date('d M Y', $model->leave_date);
                            }
                        ],							
						[
							'width'=>'10%',
							...
							'urlCreator' => function ($action, $model, $key, $index) {
                                if ($action === 'view') {
                                        $url =Yii::$app->request->baseUrl.'/admin/leave/view?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'update') {
                                        $url = Yii::$app->request->baseUrl.'/admin/leave/update?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'delete') {
