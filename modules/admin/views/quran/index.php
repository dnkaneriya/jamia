...
function submitForm()
{
    ...
    if(strvalue!=""){
        ...
        document.getElementById('delete').href = '<?php echo Yii::$app->request->baseUrl;?>/admin/quran/change?str='+strvalue+'&&field=is_deleted&&val=Y';
    }else{
		...
		<div class="box">
			...
			<div class="row p-a">
			  ...
			  <div class="col-sm-3 text-right">
				<a href="<?=Yii::$app->request->baseurl?>/admin/quran/create" id="job-export" class="btn btn-fw primary" title="Create a class"><i class="fa fa-plus"></i> Add New</a>
			  </div>
			</div>
			...
			<?php $layout = '<div class="btn-group pull-right">{toolbar}</div><div class="clearfix"></div>{items}<div class="text-right">{pager}</div>';
					...
					'layout' => $layout,
					'captionOptions'=>['title'=>'Student Quran Data'],
					'columns' => [
						[
                        ...
                        [
                            ...
                            'value'=>function($model) use($studentList){
                               return isset($studentList[$model->student_id]) ? $studentList[$model->student_id] : '-';
                            }
						],
						[
                            'width'=>'10%',
						...
						/*[
                            ...
                            //'mergeHeader'=>true,
                            'value'=>function($model){
                                if(isset($model->quran_date) && $model->quran_date != null){
                                    return date('d M Y', $model->quran_date);
                                }else{
                                    return '';
                                }
                            }
						...
						[
							...
							'urlCreator' => function ($action, $model, $key, $index) {
                                if ($action === 'view') {
                                        $url =Yii::$app->request->baseUrl.'/admin/quran/view?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'update') {
                                        $url = Yii::$app->request->baseUrl.'/admin/quran/update?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'delete') {
