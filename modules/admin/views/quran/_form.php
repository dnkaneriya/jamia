    ...
    <div class="col-lg-12">
        ...
        <div class="box-header">
            ...
            if ($model->isNewRecord) {
                echo '<h3>' . Yii::t('app', 'Add Student Quran') . '</h3>';
            } else {
                echo '<h3>' . Yii::t('app', 'Edit Student Quran') . '</h3>';
            }
            ?>
        </div>
        ...
        <div class="box-body">
            ...
            <?= $form->field($model, 'class_id')->dropDownList($classList,
                    [
                        'prompt' => 'Select Class',
                        'onchange' => '                                    $( "select#quran-subclass_id" ).html("showLoading");                                    $.post( "' . Yii::$app->urlManager->createAbsoluteUrl('admin/weightheight/get-subclass-list') . '?id=' . '"+$(this).val(),                                     function(data) {                                            $( "select#quran-subclass_id" ).html(data);                                    });'
                    ]) ?>
            
            <?=$form->field($model, 'subclass_id')->dropDownList($subclassList, 
                    [
                        'prompt' => 'Select SubClass',
                        'onchange' => '$( "select#quran-division_id" ).html("showLoading");                                $.post( "' . Yii::$app->urlManager->createAbsoluteUrl(['admin/weightheight/get-division-list']) . '?class_id=' . '"+$("#quran-class_id").val()+"&subclass_id="+$(this).val(),                                 function(data) {                                        $( "select#quran-division_id" ).html(data);                                });'
                    ]) ?>
            ...
            <?= $form->field($model, 't_year')->dropDownList($yearList,
                    [
                        'prompt' => 'Select Year',
                    ]) ?>
            
            <?= $form->field($model, 't_month')->dropDownList($monthList,
                    [
                        'prompt' => 'Select Month'
                    ]) ?>
            
            <?= $form->field($model, 'day')->dropDownList($dayList,
                    [
                        'prompt' => 'Select Day'
                    ]) ?>
         
<!--            <div class="form-group field-quran-para_no">
                <div class="row">
                    <label class="control-label col-sm-2 form-control-label" for="para_no">Para Number</label>
                    <div class="col-lg-4">
                        <input type="number" <?php if (!$model->isNewRecord) { ?> value="<?= $model->para_no ?>" <?php } ?> id="para_no" class="form-control" name="Quran[para_no]" placeholder="Para Number" min="1" max="30">
                        <div class="help-block help-block-error "></div>
                    </div>
                </div>
            </div>

            <div class="form-group field-quran-line_no">
                <div class="row">
                    <label class="control-label col-sm-2 form-control-label" for="line_no">Line Number</label>
                    <div class="col-lg-4">
                        <input type="number" <?php if (!$model->isNewRecord) { ?> value="<?= $model->line_no ?>" <?php } ?> id="line_no" class="form-control" name="Quran[line_no]" placeholder="Line Number" min="1">
                        <div class="help-block help-block-error "></div>
                    </div>
                </div>
            </div>-->
