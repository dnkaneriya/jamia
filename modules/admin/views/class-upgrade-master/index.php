        ...
        <div class="box">
            ...
            <div class="row p-a">
              ...
              <div class="col-sm-3 text-right">
                ...
                <a href="<?=Yii::$app->request->baseurl?>/admin/class-upgrade-master/create" id="job-export" class="btn btn-fw primary" title="Create a class Upgrade"><i class="fa fa-plus"></i> Add New</a> 
              </div>
            </div>
            ...
            <?php $layout = '<div class="btn-group pull-right">{toolbar}</div><div class="clearfix"></div>{items}<div class="text-right">{pager}</div>';
                    ...
                    'columns' => [
                        ...
                        [
                            ...
                            //'mergeHeader'=>true,
                            'value'=>function($model){
                                return Classes::find()->where(['id'=>$model->upgrade_id])->one()->name;
                            }
                        ],
                        [
                            'width'=>'10%',
                            'attribute'=>'upgrade_subclass_id',
                            'vAlign'=>'middle',
                            'class' => '\kartik\grid\DataColumn',
                            'headerOptions' => ['style' => 'text-align:center'],
                            'pageSummary' => false,
                            ...
                            //'mergeHeader'=>true,
                            'value'=>function($model){
                                return Subclass::find()->where(['id'=>$model->upgrade_subclass_id])->one()->sub_class;
                            }
                        ],
                        [
                            'width'=>'10%',
                            ...
                            'urlCreator' => function ($action, $model, $key, $index) {
                                if ($action === 'view') {
                                        $url =Yii::$app->request->baseUrl.'/admin/class-upgrade-master/view?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'update') {
                                        $url = Yii::$app->request->baseUrl.'/admin/class-upgrade-master/update?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'delete') {
