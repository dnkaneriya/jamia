...
$this->title = Yii::$app->params['apptitle'].' : Important Contact List';
...
function submitForm()
{
    ...
    if(strvalue!=""){
        ...
        document.getElementById('delete').href = '<?php echo Yii::$app->request->baseUrl;?>/admin/impcontact/change?str='+strvalue+'&&field=is_deleted&&val=Y';
    }else{
		...
		<div class="box">
			...
			<div class="row p-a">
			  ...
			  <div class="col-sm-3 text-right">
				<a href="<?=Yii::$app->request->baseurl?>/admin/impcontact/create" id="job-export" class="btn btn-fw primary" title="Create a class"><i class="fa fa-plus"></i> Add New</a>
			  </div>
			</div>
					...
					'columns' => [
						...
						[
							...
							'urlCreator' => function ($action, $model, $key, $index) {
                                if ($action === 'view') {
                                        $url =Yii::$app->request->baseUrl.'/admin/impcontact/view?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'update') {
                                        $url = Yii::$app->request->baseUrl.'/admin/impcontact/update?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'delete') {
