...
$this->title = Yii::$app->params['apptitle'].' : CMS Pages';
...
function submitForm()
{
    ...
    if(strvalue!=""){
        ...
        document.getElementById('delete').href = '<?php echo Yii::$app->request->baseUrl;?>/admin/cms/change?str='+strvalue+'&&field=is_deleted&&val=Y';
    }else{
		...
		<div class="box">
			...
			<div class="row p-a">
			  ...
			  <div class="col-sm-3 text-right">
			  	<?php /*
				<a href="<?=Yii::$app->request->baseurl?>/admin/cms/create" id="job-export" class="btn btn-fw primary" title="Create a class"><i class="fa fa-plus"></i> Add New</a> */ ?>
			  </div>
			</div>
			...
			<?php $layout = '<div class="btn-group pull-right">{toolbar}</div><div class="clearfix"></div>{items}<div class="text-right">{pager}</div>';
					...
					'layout' => $layout,
					'captionOptions'=>['title'=>'Page List'],
					'columns' => [
						[
                        ...
                        [
                            ...
                            'value'=>function($model){
                                return strip_tags(substr($model->content,0,100));
                            }
						],
                        /*[
						...
						[
							...
							'urlCreator' => function ($action, $model, $key, $index) {
                                if ($action === 'view') {
                                        $url =Yii::$app->request->baseUrl.'/admin/cms/view?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'update') {
                                        $url = Yii::$app->request->baseUrl.'/admin/cms/update?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'delete') {
