...
$this->title = Yii::$app->params['apptitle'].' : Student Status List';
...
function submitForm()
{
    ...
    if(strvalue!=""){
        ...
        document.getElementById('delete').href = '<?php echo Yii::$app->request->baseUrl;?>/admin/studentstatus/change?str='+strvalue+'&&field=is_deleted&&val=Y';
    }else{
		...
		<div class="box">
			...
			<div class="row p-a">
			  ...
			  <div class="col-sm-3 text-right">
				<a href="<?=Yii::$app->request->baseurl?>/admin/studentstatus/create" id="job-export" class="btn btn-fw primary" title="Create a class"><i class="fa fa-plus"></i> Add New</a>
			  </div>
			</div>
			...
			<?php $layout = '<div class="btn-group pull-right">{toolbar}</div><div class="clearfix"></div>{items}<div class="text-right">{pager}</div>';
					...
					'layout' => $layout,
					'captionOptions'=>['title'=>'Student Status List'],
					'columns' => [
						[
                        ...
                        [
                            ...
                            'mergeHeader'=>true,
                            'value'=>function($model){
                                if(isset($model->join_date) && $model->join_date != null){
                                    return date('m/d/Y', $model->join_date);
                                }else{
                                    return '';
                                }
                            }
						...
						[
                            ...
                            'mergeHeader'=>true,
                            'value'=>function($model){
                                if(isset($model->left_date) && $model->left_date != null){
                                    return date('m/d/Y', $model->left_date);
                                }else{
                                    return '';
                                }
                            }
						...
						[
							...
							'urlCreator' => function ($action, $model, $key, $index) {
                                if ($action === 'view') {
                                        $url =Yii::$app->request->baseUrl.'/admin/studentstatus/view?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'update') {
                                        $url = Yii::$app->request->baseUrl.'/admin/studentstatus/update?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'delete') {
