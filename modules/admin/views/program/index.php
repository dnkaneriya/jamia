...
$this->title = Yii::t('app', 'Program List');
...
function submitForm()
{
    ...
    if(strvalue!=""){
        document.getElementById('delete').href = '<?php echo Yii::$app->request->baseUrl;?>/admin/program/change?str='+strvalue+'&&field=is_deleted&&val=Y';
    }else{
		...
		<div class="box">
			...
			<div class="row p-a">
			  ...
			  <div class="col-sm-3 text-right">
				...
				<a href="<?=Yii::$app->request->baseurl?>/admin/program/create" id="job-export" class="btn btn-fw primary" title="Create a class"><i class="fa fa-plus"></i> Add New</a> 
			  </div>
			</div>
			...
			<?php $layout = '<div class="btn-group pull-right">{toolbar}</div><div class="clearfix"></div>{items}<div class="text-right">{pager}</div>';
					...
					'layout' => $layout,
					'captionOptions'=>['title'=>'Program List'],
					'columns' => [
						[
							...
							'value'=>function($model){
							   if($model->grnos != ''){
									$grnos=explode(",",$model->grnos);
									 return '<a id="'.$model->id.'" data-toggle="modal" data-target="#m-a-a" ui-toggle-class="fade-up" ui-target="#animate" class="disp_student" >'.count($grnos).'</a>';
								}else return 0;
						...
						[
                            ...
                            'value'=>function($model){
                                return date('d M Y', $model->p_date);
                            }
                        ],
						[
						'width'=>'10%',
							...
							'urlCreator' => function ($action, $model, $key, $index) {
                                if ($action === 'view') {
                                        $url =Yii::$app->request->baseUrl.'/admin/program/view?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'update') {
                                        $url = Yii::$app->request->baseUrl.'/admin/program/update?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'delete') {
...
<script>
	$('body').on('click','.disp_student',function(){
		$(".dispedit").html('');
		$.ajax({type: "GET",
			url: "<?php echo Yii::$app->request->baseUrl; ?>/admin/program/getstudent",
