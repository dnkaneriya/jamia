...
$student = Student::find()->where(['is_active'=>'Y','is_deleted'=>'N'])->andWhere(['<>', 'grno', ''])->all();
$studentList = ArrayHelper::map($student,'id','grno');
if(!$model->isNewRecord)
	$model->p_date = date("d/m/Y",$model->p_date);
    ...
    <div class="col-lg-12">
		<!--<section class="panel">-->
            <div class="box-header">
				<?php
					if($model->isNewRecord) {
						echo '<h3>'.Yii::t('app', 'Add Program').'</h3>';
					}else{
						echo '<h3>'.Yii::t('app', 'Edit Program').'</h3>';
					}
				?>
            </div>
            ...
            <div class="box-body">
                ...
                <?= $form->field($model, 'p_date')->textInput(['maxlength' => 255,'id'=>'p_date']); ?>
                            
<?php $model->grnos = ($model->grnos!='')?explode(",",$model->grnos):'';?>
              <?= $form->field($model, 'grnos')->dropDownList($studentList, ['class'=>'form-control',"multiple"=>"true"]) ?>               
