		...
		<div class="box">
			...
			<div class="row p-a">
			  ...
			  <div class="col-sm-3 text-right">
				...
				<a href="<?=Yii::$app->request->baseurl?>/admin/staff/create" id="job-export" class="btn btn-fw primary" title="Create a class"><i class="fa fa-plus"></i> Add New</a> 
			  </div>
			</div>
					...
					'columns' => [
						...
						[
							...
							'value'=>function($model){
								return Category::find()->where(['id'=>$model->category_id])->one()->category;
							}
                        ],
                        [
                            'width'=>'10%',
						...
						[
                            ...
                            //'mergeHeader'=>true,
							'value'=>function($model){
								return $model->sr_no != ''?$model->sr_no:'';
                            }
						...
						[
                            ...
                            //'mergeHeader'=>true,
							'value'=>function($model){
								return $model->nr_no != ''?$model->nr_no:'';
                            }
                        ],
						[
                            //'width'=>'9%',
                            ...
                            'value'=>function($model){
                                return date('d M Y', $model->join_date);
                            }
                        ],
						[
							'width'=>'10%',
							...
							'urlCreator' => function ($action, $model, $key, $index) {
                                if ($action === 'view') {
                                        $url =Yii::$app->request->baseUrl.'/admin/staff/view?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'update') {
                                        $url = Yii::$app->request->baseUrl.'/admin/staff/update?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'delete') {
