...
function submitForm()
{
    ...
    if(strvalue!=""){
        ...
        document.getElementById('delete').href = '<?php echo Yii::$app->request->baseUrl;?>/admin/guest/change?str='+strvalue+'&&field=is_deleted&&val=Y';
    }else{
		...
		<div class="box">
			...
			<div class="row p-a">
			  ...
			  <div class="col-sm-3 text-right">
				...
				<a href="<?=Yii::$app->request->baseurl?>/admin/guest/create" id="job-export" class="btn btn-fw primary" title="Create a class"><i class="fa fa-plus"></i> Add New</a> 
			  </div>
			</div>
					...
					'columns' => [
						...
						[
                            ...
                            'value'=>function($model){
                                return date('d M Y', $model->in_date);
                            }
                        ],
						[
                            //'width'=>'9%',
                            ...
                            'value'=>function($model){
                                return date('d M Y', $model->out_date);
                            }
                        ],
						[
							'width'=>'10%',
							...
							'urlCreator' => function ($action, $model, $key, $index) {
                                if ($action === 'view') {
                                        $url =Yii::$app->request->baseUrl.'/admin/guest/view?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'update') {
                                        $url = Yii::$app->request->baseUrl.'/admin/guest/update?id='.$model->id;
                                        return $url;
                                }
                                if ($action === 'delete') {
